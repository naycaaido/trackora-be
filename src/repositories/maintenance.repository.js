export function createMaintenanceRepository(prisma) {
  const createWithSideEffects = (input) =>
    prisma.$transaction(async (tx) => {
      ...
      const reportId = `mh-${uniqueSuffix}`;
      ...
      const updatedHealthScore = Math.max(
        0,
        vehicle.healthScore - penaltyBySeverity[input.severity],
      );
      const updatedStatus =
        updatedHealthScore >= 80
          ? "healthy"
          : updatedHealthScore >= 60
            ? "warning"
            : "critical";
      ...
      if (
        refreshedVehicle.status === "critical" ||
        refreshedVehicle.downtimeRisk === "high"
      ) {
        newAlerts.push({
          id: `alrt-cr-${uniqueSuffix}`,
          vehicleId: refreshedVehicle.id,
          type: "critical_risk",
          riskLevel: "high",
          message: `Vehicle ${refreshedVehicle.vehicleCode} has critical operational risk`,
          recommendation:
            refreshedVehicle.recommendation ??
